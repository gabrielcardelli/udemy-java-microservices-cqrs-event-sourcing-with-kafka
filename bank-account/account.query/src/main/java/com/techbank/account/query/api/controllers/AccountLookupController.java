...
public class AccountLookupController {
	...
	public ResponseEntity<AccountLookupResponse> getAllAccounts(){
		...
		try { 
			...
			var response = new AccountLookupResponse(MessageFormat.format("Successfully returned {0} bank accounts(s)", accounts.size()),accounts);
	...
	public ResponseEntity<AccountLookupResponse> getAccountById(@PathVariable String id){
		...
		try { 
			...
			var response = new AccountLookupResponse("Successfully returned bank account",accounts);
	...
	@GetMapping(path = "/withBalance/{type}/{balance}")
	public ResponseEntity<AccountLookupResponse> getAccountWithBalance(@PathVariable EqualityType type, @PathVariable double balance){
		
		try { 
			
			List<BankAccount> accounts = queryDispatcher.send(new FindAccountsWithBalanceQuery(type,balance));
