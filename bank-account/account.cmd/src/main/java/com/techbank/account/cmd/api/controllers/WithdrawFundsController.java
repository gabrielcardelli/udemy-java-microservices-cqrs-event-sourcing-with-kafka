...
public class WithdrawFundsController {
	...
	public ResponseEntity<BaseResponse> withdrawFunds( @PathVariable String id,@RequestBody WithdrawFundsCommand command){
		...
		try {
			...
			return new ResponseEntity<>(new BaseResponse("Withdraw funds request compled successfully."), HttpStatus.OK);
			...
			e.printStackTrace();
			var safeErrorMessage = MessageFormat.format("Erro while proccessing request to withdraw funds bank acccount for id {0}", id);
